...
jobname = 'adhesions_d=' + str(d0) + '_dH=' + str(dH) + '_dS=' + str(dS) + '_bH=' + str(bH) + '_bS=' + str(bS) + '_inittrait=' + str(init_trait_value) # if there is further subdivision within the folder 'data'. if not, leave empty
...
def GetPopDynSoln(alphaH, alphaS):
     ...
     for time in range(1,steps+1):
         ...
         xSarray.append(xS)
         xHSarray.append(xHS)
         xH += dt*(fH*xH*(1-xH/CH) - a*xH*xS + d(alphaH, alphaS)*xHS + dS*xHS + bH*xHS)
         xS += dt*(fS*xS*(1-xS/CS)- a*xH*xS + d(alphaH, alphaS)*xHS + dH*xHS + bS*xHS)
         xHS += dt*(fHS(alphaH, alphaS)*xHS*(1-xHS/CHS) + a*xH*xS - d(alphaH, alphaS)*xHS - dS*xHS - dH*xHS)
...
def MutateHost(currentH, currentS, xHstar, xSstar, xHSstar, trajectoryH, trajectoryS):
    ...
    #      print('mutantH rejected')
     ...
     mutantH_jacobian = [[fH*(1-xHstar/CH) - a*xSstar, d(mutantH, currentS) + dS + bH], 
     [a*xSstar, fHS(mutantH, currentS)*(1-xHSstar/CHS) - d(mutantH, currentS) - dH - dS]]
...
def MutateSymbiont(currentH, currentS, xHstar, xSstar, xHSstar, trajectoryH, trajectoryS):
    ...
    #      print('mutantS rejected')
     ...
     mutantS_jacobian = [[fS*(1-xSstar/CS) - a*xHstar, d(currentH, mutantS) + dH + bS], 
     [a*xHstar, fHS(currentH, mutantS)*(1-xHSstar/CHS) - d(currentH, mutantS) - dH - dS]]
...
def run_model(run_no):
     ...
     trait_trajectoryH = [init_trait_value]
     trait_trajectoryS = [init_trait_value]
     for t in range(timesteps):
         print("\n NEW EVOL TIMESTEP")
         if t%20==0:
             print("now at evolution timestep ",t)
         sigmaH = trait_trajectoryH[-1]
         sigmaS = trait_trajectoryS[-1]
         print(t, sigmaH, sigmaS)
         # calculate fixed point of population dynamics
         try:
             xHstar, xSstar, xHSstar = GetPopDynSoln(sigmaH, sigmaS)
             print('population sizes:', xHstar, xSstar, xHSstar)
         except:
             print("ode solver didn't converge. timesteps: ", len(trait_trajectoryH))
             print("trait value at this time. traitH = ", sigmaH, "traitS = ", sigmaS)
             break
         # is it feasible?
         ...
         if np.min(clocks) == clocks[0]:
             ...
             MutateHost(sigmaH, sigmaS, xHstar, xSstar, xHSstar, trait_trajectoryH, trait_trajectoryS)
             print('host mutated')
         if np.min(clocks) == clocks[1]:
             # the host clock went off first and so now we mutate the host trait
             MutateSymbiont(sigmaH, sigmaS, xHstar, xSstar, xHSstar, trait_trajectoryH, trait_trajectoryS)
...
if __name__ == '__main__':
     ...
     for run in range(no_runs): 
         ...
         data.to_csv("results/data/" + jobname + "/" + "adhesion-trajectory_" + jobname +"_run"+str(run)+".txt", index=False)
     ...
     plt.savefig("results/plots/" + jobname + "/" + "adhesion-trajectory_" + jobname  +".pdf",format='pdf')
     plt.show()
     plt.close()
     ...
     plt.ylim(0,1.1)
     plt.savefig("results/plots/" + jobname + "/" + "host-adhesion-trajectory_" + jobname +".pdf",format='pdf')
     plt.show()
     plt.close()
     ...
     plt.ylim(0,1.1)
     plt.savefig("results/plots/" + jobname + "/" + "symbiont-adhesion-trajectory_" + jobname  +".pdf",format='pdf')
     plt.show()
