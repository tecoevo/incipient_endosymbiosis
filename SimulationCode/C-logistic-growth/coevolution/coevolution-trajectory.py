...
def GetPopDynSoln(traitH, traitS):
     ...
     for time in range(1,steps+1):
         ...
         xS += dt*(fS(omegaS)*xS*(1-xS/CS) - a*xH*xS + d(alphaH, alphaS)*xHS)
...
def MutateHost(currentH, currentS, xHstar, xSstar, xHSstar, trajectoryH, trajectoryS):
    ...
    #  print('real parts of eigenvalues:', realpartsH)
     if np.max(realpartsH)>0:
         trajectoryH.append(mutantH)
        #  print('mutant H accepted')
     else:
         trajectoryH.append(currentH)
        #  print('mutant H rejected')
...
def MutateSymbiont(currentH, currentS, xHstar, xSstar, xHSstar, trajectoryH, trajectoryS):
    ...
    #  print('real parts of eigenvalues:', realpartsS)
     if np.max(realpartsS)>0:
         trajectoryS.append(mutantS)
        #  print('mutant S accepted') 
     else:
         trajectoryS.append(currentS)
        #  print('mutant S rejected')
...
def run_model(run_no):
     ...
     for t in range(timesteps):
        ...
        #  print(t, "host trait", currentH, "symbiont trait", currentS)
         ...
         if all(np.array(currentH) >= 1.0) and all(np.array(currentS) >= 1.0):
             break
         if all(np.array(currentH) >= 1.0):
             MutateSymbiont(currentH,currentS, xHstar, xSstar, xHSstar, trait_trajectoryH, trait_trajectoryS)
             continue
         if all(np.array(currentS) >= 1.0):
