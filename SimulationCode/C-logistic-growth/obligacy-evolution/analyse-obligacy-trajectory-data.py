...
allrunsH = []
allrunsS = []
srcdir = 'a=' + str(a) + '_rHS=' + str(rHS) + '_CHS=' + str(CHS) # if there is further subdivision within the folder 'data'. if not, leave empty
...
plt.savefig("results/plots/" + srcdir + "/" +  "obligacy-trajectory_a="+str(a)+"_rHS="+str(rHS) + "_CHS=" + str(CHS) +".pdf",format='pdf')
plt.show()
plt.close()
