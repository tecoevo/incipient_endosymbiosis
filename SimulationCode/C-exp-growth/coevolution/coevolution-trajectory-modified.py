...
# in this script, we modify the basic script coevolution-trajectory.py to understand why there \ 
# there is a bias towards mutual dependence and away from reproductive cohesion.
# In particular, all modification takes place in the functions fH, fS, fHS, and d. 

# default: (this combination gives rise to the bias in the main text)
# fH = rH*(1-omegaH)
# fS = rS*(1-omegaS)
# fHS = rHS*omegaH*omegaS*alphaH*alphaS
# d = d0*(1-alphaH*alphaS)

# there are four possible tests to run: independent growth can either depend only on 
# (i) own obligacy or (ii) both, 
# and association-dissociation rates can either depend only on (I) one adhesion or (II) both. 
# the default above has independent growth depending only on own obligacy and a-d rates depending on both adhesions.
# we'll use shorthand i-II for this combination (for obvious reasons, see previous sentences). 
# so we are left with three other tests to perform. 

# which test is performed depends on which of the following flags is set to True:
# below code will malfunction if more than one is set to True. 
defaultiII=False
iI = False
iiI = False
iiII = True

# there is another note to be made here: when we make a-d rates depend only one trait, we pick 
# the symbiont trait without loss of generality. This does not have a qualitative effect, and this 
# can be easily checked using this script.
# further, we fix the effect of the obligacies and adhesions on f_C; this does not change because our mental 
# model of an endosymbiosis still stays the same - increasing both of these traits improves collective growth rate.
# we are instead changing the effect of this increased collective on the independent individuals. 
# We (perhaps artificially) change only the effect of obligacies on independent growth, and the effect of 
# adhesions on a-d rates to understand their effect.   

verbose = True

# default: independent growth depending only on own obligacy and a-d rates depending on both adhesions
# result: bias towards mutual dependence
 
if defaultiII == True and iI == False and iiI == False and iiII == False:
    print("default accessed")
    def fH(omegaH, omegaS):
        return rH*(1.-omegaH)

    def fS(omegaS, omegaH):
        return rS*(1.-omegaS)
...
# iI: independent growth depending only on own obligacy and a-d rates depending on one adhesion. 
# what happens: bias towards mutual dependence, but lesser. The reason can be understood by looking at 
# the graphs for omegaH-alphaH and omegaS-alphaS. 
# as given below, there is a benefit for both to increase obligacy, and there is the *same* kind of benefit
# for the symbiont to increase its adhesion since d = d0*(1-alphaS).
# there is a smaller benefit for the host to increase its adhesion because an increased host adhesion
# only increases fHS but doesn't decrease d. 
# Therefore, omegaH evolves faster than alphaH BUT omegaS does not evolve faster than alphaS. 
# 
# If instead d=d0(1-alphaH), there is no bias between omegaH and alphaH whereas omegaS evolves faster than alphaS.  
    
if defaultiII == False and iI == True and iiI == False and iiII == False:
    print("iI accessed")
    def fH(omegaH, omegaS):
        return rH*(1.-omegaH)
...
# iiI: independent growth depending on both obligacies and a-d rates depending on only one adhesion. 
# result: bias towards reproductive cohesion. This is because the benefit of increasing the adhesion is 
# strong now than that of increasing obligacy (because the functional effect of increasing adhesion does
# not depend also on the other adhesion. ) 
# There is also a slight bias towards dependence in the host because increasing adhesion for the host does
# not increase d. 
# If instead d=d0(1-alphaH), there would be bias towards dependence in the symbiont and a bias towards 
# cohesion in the host. 
 
if defaultiII == False and iI == False and iiI == True and iiII == False:
    print("iiI accessed")
    def fH(omegaH, omegaS):
        return rH*(1.-omegaH*omegaS)

    def fS(omegaS, omegaH):
        return rS*(1.-omegaS*omegaH)
...
# iiII: independent growth depending on both obligacies and a-d rates depending on both adhesions. 
# result: no bias, as expected. 

if defaultiII == False and iI == False and iiI == False and iiII == True:
    print("iiII accessed")
    def fH(omegaH, omegaS):
        return rH*(1.-omegaH*omegaS)
...
rH = 8.0
rS = 2*rH # how large is the pure symbiont growth rate as compared to the pure host growth rate?
...
def Objective(traitH, traitS, id):
     ...
     alphaS = traitS[1]
     if id=='H':
         return (fH(omegaH, omegaS)/a)*(1-(d(alphaH, alphaS))/fHS(omegaH, omegaS, alphaH, alphaS))
     if id=='S':
         return (fS(omegaS, omegaH)/a)*(1-(d(alphaH, alphaS))/fHS(omegaH, omegaS, alphaH, alphaS))
...
for run_no in range(no_runs): # different random trajectories
     ...
     for t in range(timesteps):
         ...
         fH_here = fH(omegaH, omegaS)
         fS_here = fS(omegaS, omegaH)
